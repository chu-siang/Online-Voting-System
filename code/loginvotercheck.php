...
if (isset($_POST['voter_ID_Number'])) {
	...
	if (empty($vote_ID)) {
		header("Location: loginvoter.php?error=Vote_ID is required");
	    exit();
	}else{
		...
		if (mysqli_num_rows($result) === 1) {
            ...
            if ($row['vote_key'] === $key && $row['voter_ID_Number'] === $vote_ID) {
				...
				echo "Your are not allowed in this Cosign!!";
				header("Location: loginvoter.php?error=You are not allowed in this Cosign!!");
		        exit();
			}else{
				echo "Incorect User Information!!";
				header("Location: loginvoter.php?error=Incorect User Information!!");
		        exit();
			}
		}else{
			header("Location: loginvoter.php?error=Incorect User Information!!");
