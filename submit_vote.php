...
if (isset($_POST['selected_candidate'])) {
    ...
    if (mysqli_num_rows($result) === 1) {
        ...
        if ($row['is_vote'] > 0) {
            ...
            echo "You have voted!!!";
            header("Location: votepage.php?error=You have voted!!");
            // header("Location: home.php");
            exit();
        }else if ($row['vote_key'] === $key && $row['voter_ID_Number'] === $vote_ID) {
            $sql3 = "SELECT * FROM candidate WHERE candidate_Name='$selectedCandidate'";
            $result3 = mysqli_query($conn, $sql3);
            $row3 = mysqli_fetch_assoc($result3);
            if ($row3['candidate_Name'] === $selectedCandidate) {
                $sql4 = "UPDATE candidate SET candidate_Number = candidate_Number + 1 WHERE candidate_Name='$selectedCandidate' AND vote_key='$key'";
                $result4 = mysqli_query($conn, $sql4);
                $sql5 = "SELECT * FROM candidate WHERE candidate_Name='$selectedCandidate'";
            ...
            $sql2 = "UPDATE voter SET is_vote = 1 WHERE vote_key='$key' AND voter_ID_Number='$vote_ID'";
            $result2 = mysqli_query($conn, $sql2);
            echo "Successfully voted!!";
            header("Location: votepage.php?success= Successfully voted!!");
            exit();
        }else{
            header("Location: votepage.php?error=Incorect User name or password!!");
            exit();
        }
    }else{
        header("Location: votepage.php?error=Incorect User name or password");
